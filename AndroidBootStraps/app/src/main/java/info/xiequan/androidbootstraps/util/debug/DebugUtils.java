package info.xiequan.androidbootstraps.util.debug;
...
public class DebugUtils {
    ...
    public synchronized static void writeLog(byte log[]) {
        ...
        for (int i = 0; i < log.length; i++) {
            slog += " " + DebugUtils.toHex(log[i]);
        }
        DebugUtils.writeLogFile(slog);
    }
    ...
    public static String getDeviceInfo(Context context) {
        ...
        try {
            ...
            if (AndroidUtils.hasGingerbread()) {
                builder.append("安装时间:").append(AndroidUtils.getFormatDateString(info.firstInstallTime))
                        .append("\n");
            }
        } catch (Exception e1) {
        }
    ...
    public synchronized static void buildLogMessage(Throwable ex) {
        ...
        if (ex != null) {
            builder.append("\n\n").append("------ 程序错误信息 -------").append("\n")
                    .append(DebugUtils.getLogStyle(DebugUtils.ERROR, TAG, "", ex));
        }
        long currentTime = System.currentTimeMillis();
